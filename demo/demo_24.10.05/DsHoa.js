...
export default function DsHoa({ navigation,route }) {
const [hoachons,layhoachon]=useState([]);
const[tenloai,gantenloai]=useState(route.params.tenloai);
const DocBangHoatheoLoai=()=>{
  db.transaction((tx) => {
    tx.executeSql(
      'SELECT * FROM tblhoa where tenloai=?', [tenloai],
      (tx, results) => {
        var temp = [];
       ...
       /* const hoas =[{
          ...
          */
          const ItemView = ({item}) => {
            return (
              <View>
                <Text onPress={()=>{navigation.navigate('ChitietHoa',{hoachon:item})}}>Tên Hoa :{item.tenhoa}</Text>
